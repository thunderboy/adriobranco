...
if (!function_exists('woocommerce_template_single_price')) {
	function woocommerce_template_single_price( $post, $_product ) {
		?><p itemprop="price" class="price">Price: <?php echo $_product->get_price_html(); ?></p><?php
	}
}
		...
		<form action="<?php echo esc_url( $_product->add_to_cart_url() ); ?>" class="variations_form cart" method="post">
			<table class="variations">
				...
				<?php foreach ($attributes as $name => $options) : ?>
					<tr>
						<td><label for="<?php echo sanitize_title($name); ?>"><?php echo $woocommerce->attribute_label($name); ?><?php echo __(': ', 'woothemes') ?></label></td>
			...
			<div class="variations_wrap">
				...
				<div class="variations_button">
					...
					<div class="qty"><?php _e('Quantity:', 'woothemes'); ?></div>
					<div class="quantity"><input name="quantity" value="1" size="4" title="Qty" class="input-text qty text" maxlength="12" /></div>
...
if (!function_exists('woocommerce_product_description_panel')) {
	function woocommerce_product_description_panel() {
		...
		echo '<h2>' . apply_filters('woocommerce_product_description_heading', __('Details:', 'woothemes')) . '</h2>';
	...
	function woocommerce_cart_totals() {
		...
		if ( !$woocommerce->shipping->enabled || $available_methods || !$woocommerce->customer->get_shipping_country() || !isset($_SESSION['calculated_shipping']) || !$_SESSION['calculated_shipping'] ) :
			...
			<table cellspacing="0" cellpadding="0">
				...
				<a href="<?php echo esc_url( $woocommerce->cart->get_checkout_url() ); ?>" class="checkout-button button alt"><?php _e('Checkout', 'woothemes'); ?></a>
			<div class="clear"></div>
			<?php
		else :
			?>
